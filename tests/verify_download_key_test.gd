...
func _ready():
	...
	# Try to load secrets from file (only game_id now)
	_load_secrets_file()
	...
	if not secrets_loaded:
		...
		status_label.text = "Status: Enter Game ID and download key to test"
	else:
		download_key_input.text = download_input_key
...
func _load_secrets_file():
	"""Try to load project data from the file and populate settings"""
	...
	if file:
		...
		if error == OK:
			...
			if data.has("game_id"):
				...
				secrets_loaded = true
				print("[TEST] Game ID loaded successfully from file")
				return
			else:
				print("[TEST] Error: secrets file missing required field (game_id)")
		else:
