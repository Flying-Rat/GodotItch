...
func _ready():
	print("=== GodotItch Access Method & Basic Functionality Test ===")
	
	# Test 1: Check if autoload exists immediately
	test_immediate_autoload_access()
	
	# Test 2: Test GodotItch class access
...
func test_godot_itch_class_access():
	print("\n--- Test 2: GodotItch Class Access ---")
	
	# Check if GodotItch class is available
	if ClassDB.class_exists("GodotItch"):
		print("✓ GodotItch class exists in ClassDB")
	else:
		print("! GodotItch class not in ClassDB (this is normal for custom classes)")
	
	# Test static methods
	var plugin_info = GodotItch.get_plugin_info()
	print("✓ GodotItch.get_plugin_info() works")
	...
	print("✓ GodotItch.get_verification_status() works")
	print("  Verified: ", status.verified)
	
	var validation = GodotItch.validate("test_key_123456789012345678901")
	print("✓ GodotItch.validate() works")
...
func test_signal_connections():
	...
	# Method 2: GodotItch class connection
	var callable = Callable(self, "_dummy_signal_handler")
	GodotItch.connect_verification_started(callable)
	print("✓ GodotItch signal connection works")
	GodotItch.disconnect_verification_started(callable)
